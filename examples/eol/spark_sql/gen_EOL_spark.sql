...
with
    death_dates as (
        select
            ...
            p.death_datetime
        from
            parquet.`datasets/parquet/person.parquet` p
    ),
    eligible_people as (
        select p.person_id
        from parquet.`datasets/parquet/person.parquet` p
        where extract(
    ...
    death_training_elig_counts as (
        ...
        from parquet.`datasets/parquet/observation_period.parquet` o
    ...
    death_testperiod_elig_counts as (
        ...
        from parquet.`datasets/parquet/observation_period.parquet` p
        inner join
